...
mask_map = rast("Polaris_SoilData/Processed/ccr_mask.tif")
plot(mask_map)

# maskPolygon <- polygon_from_extent(ext(mask_map), proj4string='+proj=utm +datum=NAD83 +zone=11')

NLCD <- get_nlcd(template=mask_map, label='VEPIIN', year = 2019, force.redo = T, extraction.dir = paste0("rhutils_trials/NLCD/"))
...
dev.copy2pdf(file = file.path("rhutils_trials/NLCD/", "NLCD_baseline.pdf"), width = 10, height = 6)
...
NLCD_mask_reclass = terra::classify(NLCD_mask,
                                    data.frame(c(11, 21, 22, 23, 24, 31, 41, 42, 43, 52, 71, 81, 82, 90, 95),
                                               c(1,  1,  1,  1,   1,  1,  3,  4,  3,  2,  2,  2,  2,  2,  2) ))

#updated trying to match numbers below
# NLCD_mask_reclass = terra::classify(NLCD_mask,
#                                     data.frame(c(11, 21, 22, 23, 24, 31, 41, 42, 43, 52, 71, 81, 82, 90, 95),
#                                                c(31, 31, 31, 31, 31, 31,  7,  7,  7,  7, 50, 50, 50, 50, 50) ))
...
writeRaster(NLCD_mask_reclass, "rhutils_trials/NLCD/NLCD_veg_cover.tif", overwrite = T)
