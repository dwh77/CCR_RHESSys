...
#cmd3 = sprintf("-st 1950 10 1 1 -ed 2024 10 1 1 -b -g") #DWH changed year
...
```{r}
getwd()
# #read in files that were run on 20oct 12:48pm. Test name is legacy for command line above but run with these inputs
# rhessys_ver = "../../rhessys7.5" #new to point backwards to model
# worldfile = "worldfiles/hpb.world"
# header = "worldfiles/hpb.hdr"
# flowtable = "flowtables/hpb.flow"
...
hpb_test = readin_rhessys_output("out/HPBtest")
#DWH trials 
names(hpb_test)
...
hpb_test_bd |> 
  ...
  select(Date, basinID, streamflow, baseflow, precip, evap, trans, et, precip_minus_et) |> 
  pivot_longer(-c(1:2)) |> 
  ggplot(aes(x = Date, y = value))+
  geom_point()+
  #ylim(0,1)+
